document.addEventListener('DOMContentLoaded', () => {
    ...
    function extractSKUs(rawText) {
        ...
        const regexYA = /(?:YA|ya|ＹＡ|ｙａ)[-ー━‐_~=・.]*(s[0-9a-zA-Z]{5,15})/gi;
        let matchYA;
        ...
        // 4. Match YFM-z... (Yahooフリマ)
        const regexYFM = /(?:YFM|yfm|ＹＦＭ|ｙｆｍ)[-ー━‐_~=・.]*(z[0-9a-zA-Z]{5,15})/gi;
        let matchYFM;
    ...
    function renderResults(results) {
        if (results.length === 0) {
            renderError("有効なSKUを認識できませんでした。<br><small style='color:#EF4444; margin-top:4px; display:block;'>対応形式: YA-sxxxxxxxxx または YFM-zxxxxxxxxx</small>");
            return;
        }
